...
class StatTest(ABC):
    @abstractmethod
    def min_sample_size(self, control: np.ndarray, test: np.ndarray, power: float, alpha: float) -> int:
        """
    ...
    @abstractmethod
    def calculate_power(self, control: np.ndarray, test: np.ndarray, alpha: float) -> float:
        """
